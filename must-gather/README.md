...
Also, several resources like `clusterlogging`, `clusterlogforwarder`, `installplans`, `subscriptions`, `clusterserviceversions`, etc. are now collected by `oc adm inspect` command in the diferent `namespaces/[namespace_name]/` directories and not in `cluster-logging/clo`. This allows to use tools like [`omc`](https://github.com/gmeghnag/omc/) to work with those resources in a similar way than in a live cluster with `oc` commands.

The `deployments`, `daemonsets` and `secrets` can be also found in `namespaces/[namespace_name]/` and can be seen with [`omc`](https://github.com/gmeghnag/omc/) (and run commands like `omc get deployments -n [namespace_name] -o wide`).